    ...
    fun sendText(targetFingerprint: String, message: String) {
        ...
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                Log.d(tag, "Message $msgId queued as PENDING (not connected to $targetFingerprint)")
            } catch (e: Exception) {
    ...
    fun sendImage(targetFingerprint: String, imageBytes: ByteArray) {
        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                if (targetFingerprint != peerFingerprint) {
                    Log.d(tag, "Image $messageId queued as PENDING (not connected to $targetFingerprint)")
                    return@launch
                }
        ...
        try {
            ...
            if (text.startsWith("{")) {
                ...
                if (json.optString("type") == "handshake") {
                    Log.i(tag, "Received unencrypted handshake, processing...")
